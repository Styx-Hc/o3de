 ...
 */
#define AZ_ENSURE_STRING_OUTCOME_CONDITION(cond, ...)                                                                                      \
    if (!(cond))                                                                                                                           \
    {                                                                                                                                      \
...
#define AZ_ENSURE_STRING_OUTCOME(outcome)                                                                                                  \
...
#define STRING_OUTCOME_ERROR(error) AZ::Failure(AZStd::string(error))
    ...
    static const char* s_SupportedImageExtensions[] = { "*.tif", "*.tiff", "*.png", "*.bmp", "*.jpg", "*.jpeg",
                                                        "*.tga", "*.gif",  "*.dds", "*.exr", "*.pgm" };
    ...
    enum class MipGenType : AZ::u32
    {
        ...
        box, // Also called 'average'. When shrinking images it will average, merge the pixels together.
